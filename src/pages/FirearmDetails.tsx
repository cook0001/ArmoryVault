...
import { Firearm, MaintenanceLog, MaintenanceScheduleItem, Ammo, Accessory, CustomSchedulePreset } from '../types';
import { ArrowLeft, Edit, Trash2, DollarSign, ClipboardList, FileText, Upload, Crosshair, PlusCircle, Printer, Wrench, CheckCircle, AlertTriangle, Sparkles, Clock, Sliders, Check, Bookmark, Star, Calendar, ShieldCheck } from 'lucide-react';
import { AccessoryModal } from '../components/AccessoryModal';
import { Lightbox } from '../components/Lightbox';
import { MAINTENANCE_PROFILES, detectFirearmScheduleProfile, createScheduleItemsFromProfile, MaintenanceProfile } from '../utils/maintenancePresets';
...
export const FirearmDetails = () => {
  ...
  const [scheduleForm, setScheduleForm] = useState<{ id?: string; task_name: string; interval_rounds: number; enableDays?: boolean; interval_days?: number; notes: string }>({ task_name: '', interval_rounds: 3000, enableDays: false, interval_days: 90, notes: '' });
  ...
  const [completionForm, setCompletionForm] = useState({ action_performed: '', part_details: '', part_manufacturer: '', cost: '', date: new Date().toISOString().split('T')[0], notes: '' });
  ...
  const handleDocumentSelect = async () => {
    ...
    try {
      ...
      if (doc && doc.path) {
        const updatedDocs = [...(firearm.documents || []), { name: doc.name, path: doc.path, date_added: new Date().toISOString().split('T')[0] }];
  ...
  const handleSaveLog = async (e: React.FormEvent) => {
    ...
    const newLogData: MaintenanceLog = {
      ...
      repaired_part: (logForm.type === 'Modification' || logForm.type === 'Repair' || logForm.type === 'Other') ? logForm.repaired_part : undefined,
      part_manufacturer: (logForm.type === 'Modification' || logForm.type === 'Repair' || logForm.type === 'Other') ? logForm.part_manufacturer : undefined,
      installed_part_details: (logForm.type === 'Modification' || logForm.type === 'Repair' || logForm.type === 'Other') ? logForm.installed_part_details : undefined,
      cost: (logForm.type === 'Modification' || logForm.type === 'Repair' || logForm.type === 'Other') ? (Number(logForm.cost) || 0) : undefined,
  ...
  const handleSaveSchedule = async (e: React.FormEvent) => {
    ...
    const totalRounds = firearm.logs?.filter(l => l.type === 'Range').reduce((sum, l) => sum + (Number(l.rounds_fired) || 0), 0) || 0;
    const currentSchedules = [...(firearm.maintenance_schedules || [])];
    const intervalDays = scheduleForm.enableDays && scheduleForm.interval_days ? Number(scheduleForm.interval_days) : undefined;
    ...
    setScheduleForm({ task_name: '', interval_rounds: 3000, enableDays: false, interval_days: 90, notes: '' });
  };

  const handleDeleteSchedule = async (taskId: string) => {
    if (!firearm || !window.confirm('Are you sure you want to delete this maintenance schedule task?')) return;
  ...
  const handleLoadPreset = async (profileOrCustomId: string, mode: 'replace' | 'append' = 'replace', isCustom: boolean = false) => {
    if (!firearm) return;
    const totalRounds = firearm.logs?.filter(l => l.type === 'Range').reduce((sum, l) => sum + (Number(l.rounds_fired) || 0), 0) || 0;
    ...
    if (isCustom) {
      ...
      const profile = MAINTENANCE_PROFILES[profileOrCustomId] || detectFirearmScheduleProfile(firearm);
      newItems = createScheduleItemsFromProfile(profile, totalRounds);
    }

    let updatedSchedules: MaintenanceScheduleItem[];
    if (mode === 'append' && firearm.maintenance_schedules && firearm.maintenance_schedules.length > 0) {
      const existingNames = new Set(firearm.maintenance_schedules.map(s => s.task_name.toLowerCase().trim()));
      const nonDuplicates = newItems.filter(item => !existingNames.has(item.task_name.toLowerCase().trim()));
  ...
  const handleSaveAsTemplate = async (e: React.FormEvent) => {
    ...
    const newPreset: CustomSchedulePreset = {
      ...
      description: customPresetForm.description.trim() || `${firearm.make} ${firearm.model} custom schedule`,
  ...
  const handleSaveTaskCompletion = async (e: React.FormEvent) => {
    ...
    if (window.api && window.api.completeMaintenanceTask) {
      const partDetailStr = completionForm.part_details ? `${completionForm.part_manufacturer ? completionForm.part_manufacturer + ' ' : ''}${completionForm.part_details}` : completionForm.action_performed;
  ...
  const handleQuickClean = async () => {
    ...
    const updatedSchedules = (firearm.maintenance_schedules || []).map(s => {
      if (s.task_name.toLowerCase().includes('clean')) {
        return { ...s, last_performed_rounds: totalRounds, last_performed_date: new Date().toISOString().split('T')[0] };
      }
      return s;
    });
    ...
    <>
      ...
      <div className="page-header">
        ...
        <div className="header-actions" style={{ gap: '0.5rem', background: 'rgba(0,0,0,0.2)', padding: '0.4rem', borderRadius: '12px', border: '1px solid var(--border-light)' }}>
          ...
          <button className="btn-secondary" onClick={() => setIsDossierOpen(true)} style={{ padding: '0.4rem 0.8rem', fontSize: '0.85rem', background: 'transparent', border: 'none', color: '#38bdf8', display: 'flex', alignItems: 'center', gap: '0.3rem' }} title="Generate full printable gunsmith service dossier & logbook">
      ...
      <div className="details-card" style={{ marginTop: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '1rem', flexWrap: 'wrap', gap: '0.75rem' }}>
          <div>
            <h3 style={{ margin: 0, border: 'none', padding: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Wrench size={20} style={{ color: 'var(--accent)' }} /> Proactive Maintenance & Service Schedules
            </h3>
            <p style={{ margin: '0.25rem 0 0', color: 'var(--text-secondary)', fontSize: '0.85rem' }}>
              Track part replacements and service intervals based on actual rounds fired and elapsed calendar days.
            </p>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            <button className="btn-secondary" onClick={handleQuickClean} title="Log a quick field strip & cleaning">
            ...
            {firearm.maintenance_schedules && firearm.maintenance_schedules.length > 0 && (
              <button className="btn-secondary" onClick={() => {
                setCustomPresetForm({ name: `${firearm.make} ${firearm.model} Schedule`, description: `${firearm.action_type || ''} custom maintenance schedule` });
                setIsSavePresetModalOpen(true);
              }} title="Save this firearm's schedules as a reusable template">
                <Bookmark size={16} style={{ color: '#fbbf24' }} /> Save as Template
              </button>
            )}
            <button className="btn-secondary" onClick={() => { setSelectedPresetId(detectFirearmScheduleProfile(firearm).id); setIsPresetModalOpen(true); }} title="Browse standard action profiles and custom templates">
        ...
        {(!firearm.maintenance_schedules || firearm.maintenance_schedules.length === 0) ? (
          <div style={{ textAlign: 'center', padding: '2.5rem 1.5rem', background: 'rgba(0,0,0,0.15)', borderRadius: '12px', border: '1px dashed var(--border-light)' }}>
            <div style={{ maxWidth: '680px', margin: '0 auto' }}>
              <Wrench size={36} style={{ color: 'var(--accent)', opacity: 0.8, marginBottom: '0.75rem' }} />
              <h4 style={{ margin: '0 0 0.5rem', fontSize: '1.1rem' }}>No Maintenance Schedules Configured</h4>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '1.25rem', lineHeight: 1.5 }}>
                Proactive schedules track cleaning, wear part replacements, and springs automatically based on rounds fired or calendar days elapsed.
              </p>
              ...
              {(() => {
                ...
                return (
                  <div style={{ background: 'rgba(56, 189, 248, 0.08)', border: '1px solid rgba(56, 189, 248, 0.25)', borderRadius: '10px', padding: '1rem 1.25rem', marginBottom: '1.25rem', textAlign: 'left' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem', flexWrap: 'wrap', gap: '0.4rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <span style={{ background: '#38bdf8', color: '#0f172a', fontSize: '0.7rem', fontWeight: 800, padding: '0.15rem 0.5rem', borderRadius: '4px', textTransform: 'uppercase' }}>
                          Recommended Preset
                        </span>
                        <strong style={{ color: 'var(--text-primary)', fontSize: '0.95rem' }}>{detected.name}</strong>
                      </div>
                      <span style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>
                        Tailored for: {firearm.action_type ? `Action: "${firearm.action_type}"` : `${firearm.make} ${firearm.model} (${firearm.caliber})`}
                      </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                      {detected.tasks.map((t, idx) => (
                        <span key={idx} style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.08)', padding: '0.2rem 0.6rem', borderRadius: '6px', fontSize: '0.75rem', color: 'var(--text-primary)' }}>
                          {t.task_name} <span style={{ color: 'var(--accent)', fontWeight: 600 }}>({t.interval_rounds.toLocaleString()} rds{t.interval_days ? ` / ${t.interval_days}d` : ''})</span>
                        </span>
                      ))}
                    </div>
                  </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
                <button className="btn-primary" onClick={() => handleLoadPreset(detectFirearmScheduleProfile(firearm).id, 'replace')}>
                  <Sparkles size={16} /> Load Standard Schedule ({detectFirearmScheduleProfile(firearm).name})
                </button>
                <button className="btn-secondary" onClick={() => { setSelectedPresetId(detectFirearmScheduleProfile(firearm).id); setIsPresetModalOpen(true); }}>
                  <Sliders size={16} /> Browse Presets & Templates ({Object.keys(MAINTENANCE_PROFILES).length + customPresets.length})...
                </button>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1rem' }}>
            {firearm.maintenance_schedules.map((schedule) => {
              const totalLifetimeRounds = firearm.logs?.filter(l => l.type === 'Range').reduce((sum, l) => sum + (Number(l.rounds_fired) || 0), 0) || 0;
              const roundsSinceLast = Math.max(0, totalLifetimeRounds - (schedule.last_performed_rounds || 0));
              const pctRounds = Math.min(100, Math.round((roundsSinceLast / (schedule.interval_rounds || 1)) * 100));
              ...
              if (schedule.interval_days && schedule.last_performed_date) {
                ...
                pctDays = Math.min(100, Math.round((daysSinceLast / schedule.interval_days) * 100));
              }

              const isDueByRounds = roundsSinceLast >= schedule.interval_rounds;
              const isDueByDays = schedule.interval_days && daysSinceLast !== null && daysSinceLast >= schedule.interval_days;
              ...
              return (
                <div key={schedule.id} style={{ background: isDue ? 'rgba(239, 68, 68, 0.08)' : 'rgba(0,0,0,0.2)', border: isDue ? '1px solid rgba(239, 68, 68, 0.4)' : '1px solid var(--border-light)', borderRadius: '10px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: '1.05rem', display: 'flex', alignItems: 'center', gap: '0.4rem', color: 'var(--text-primary)' }}>
                        {schedule.task_name}
                        {isDue && (
                          <span style={{ background: '#ef4444', color: '#fff', fontSize: '0.65rem', fontWeight: 800, padding: '0.15rem 0.45rem', borderRadius: '4px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                            SERVICE DUE
                          </span>
                        )}
                      </div>
                      <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
                        Every {schedule.interval_rounds.toLocaleString()} rds{schedule.interval_days ? ` or ${schedule.interval_days} days` : ''} &bull; Last: {schedule.last_performed_rounds.toLocaleString()} rds ({schedule.last_performed_date || 'N/A'})
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: '0.3rem' }}>
                      <button className="btn-icon" onClick={() => {
                        ...
                        setIsScheduleModalOpen(true);
                      }} style={{ padding: '0.3rem' }} title="Edit Task"><Edit size={14} /></button>
                      <button className="btn-icon" onClick={() => handleDeleteSchedule(schedule.id)} style={{ padding: '0.3rem', color: 'var(--danger)' }} title="Delete Task"><Trash2 size={14} /></button>
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', marginBottom: '0.2rem', color: 'var(--text-secondary)' }}>
                        ...
                        <span style={{ fontWeight: 600, color: pctRounds >= 100 ? '#ef4444' : 'var(--text-primary)' }}>
                          {roundsSinceLast.toLocaleString()} / {schedule.interval_rounds.toLocaleString()} rds ({pctRounds}%)
                        </span>
                      </div>
                      <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: '6px', height: '6px', overflow: 'hidden' }}>
                        <div style={{ width: `${pctRounds}%`, background: pctRounds >= 100 ? '#ef4444' : 'var(--accent)', height: '100%', transition: 'width 0.3s ease' }} />
                      </div>
                    </div>
                    ...
                    {schedule.interval_days && daysSinceLast !== null && (
                      <div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.75rem', marginBottom: '0.2rem', color: 'var(--text-secondary)' }}>
                          ...
                          <span style={{ fontWeight: 600, color: (pctDays || 0) >= 100 ? '#ef4444' : 'var(--text-primary)' }}>
                            {daysSinceLast} / {schedule.interval_days} days ({pctDays}%)
                          </span>
                        </div>
                        <div style={{ background: 'rgba(255,255,255,0.08)', borderRadius: '6px', height: '6px', overflow: 'hidden' }}>
                          <div style={{ width: `${pctDays}%`, background: (pctDays || 0) >= 100 ? '#ef4444' : '#38bdf8', height: '100%', transition: 'width 0.3s ease' }} />
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div style={{ marginTop: 'auto', paddingTop: '0.5rem', display: 'flex', justifyContent: 'flex-end' }}>
                    <button
                      ...
                      style={{ padding: '0.45rem 0.85rem', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.4rem', width: '100%', justifyContent: 'center' }}
                    >
            ...
            <div key={log.id} style={{ background: 'rgba(0,0,0,0.2)', padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--border-light)', position: 'relative', overflow: 'hidden' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '0.5rem' }}>
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button className="btn-icon" onClick={() => handleEditLog(log)} style={{ padding: '0.2rem' }} title="Edit"><Edit size={16} /></button>
          ...
          <div className="modal" style={{ maxWidth: '500px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.75rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <h3 style={{ margin: 0 }}>{editingScheduleId ? 'Edit Maintenance Schedule' : 'New Maintenance Schedule'}</h3>
              </div>
              <button className="btn-icon" onClick={() => setIsScheduleModalOpen(false)}>×</button>
            </div>
            <form onSubmit={handleSaveSchedule} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setScheduleForm({ ...scheduleForm, interval_rounds: parseInt(e.target.value) || 0 })}
                  required
                />
                <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', marginTop: '0.3rem' }}>
                  Alerts will trigger every {scheduleForm.interval_rounds || 0} rounds fired through this firearm.
                </div>
              </div>
                ...
                {scheduleForm.enableDays && (
                  <div style={{ marginTop: '0.75rem' }}>
                    ...
                    <input
                      ...
                      onChange={(e) => setScheduleForm({ ...scheduleForm, interval_days: parseInt(e.target.value) || 0 })}
                      required={scheduleForm.enableDays}
                    />
                    <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.3rem' }}>
                      Task will alert when <strong>either</strong> {scheduleForm.interval_rounds || 0} rounds OR {scheduleForm.interval_days || 0} days have elapsed.
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="modal-actions" style={{ marginTop: '0.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border-light)' }}>
                ...
                <button type="submit" className="btn-primary">{editingScheduleId ? 'Update Schedule' : 'Save Task'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isTaskCompletionModalOpen && completingTask && (
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: '580px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.75rem' }}>
              ...
              <button className="btn-icon" onClick={() => setIsTaskCompletionModalOpen(false)}>×</button>
            </div>
            <form onSubmit={handleSaveTaskCompletion} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '0.4rem', fontWeight: 600, fontSize: '0.9rem' }}>
                  Action Performed
                </label>
                <input
                  ...
                  onChange={(e) => setCompletionForm({ ...completionForm, action_performed: e.target.value })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setCompletionForm({ ...completionForm, part_manufacturer: e.target.value })}
                  />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.85rem', fontWeight: 500 }}>
                  ...
                  <input
                    ...
                    onChange={(e) => setCompletionForm({ ...completionForm, part_details: e.target.value })}
                  />
                </div>
              </div>
              ...
              <div className="modal-actions" style={{ marginTop: '0.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border-light)' }}>
                <button type="button" className="btn-secondary" onClick={() => setIsTaskCompletionModalOpen(false)}>Cancel</button>
                <button type="submit" className="btn-success" style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
      ...
      {isPresetModalOpen && (
        <div className="modal-overlay" style={{ zIndex: 1100 }}>
          <div className="modal" style={{ maxWidth: '920px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.75rem' }}>
              ...
              <button className="btn-icon" onClick={() => setIsPresetModalOpen(false)}>×</button>
            </div>

            {/* Category Tabs */}
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.5rem' }}>
              <button
                type="button"
              ...
              >
                <Bookmark size={14} style={{ color: '#fbbf24' }} /> My Custom Templates ({customPresets.length})
              </button>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: '1.5rem', overflowY: 'auto', paddingRight: '0.5rem', flex: 1, minHeight: '340px' }}>
              {/* Profile Selector List */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.2rem' }}>
                ...
                {presetTab === 'profiles' ? (
                    ...
                    return (
                      ...
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <strong style={{ fontSize: '0.95rem', color: isSelected ? '#fff' : 'var(--text-primary)' }}>
                            {profile.name}
                          </strong>
                          {isDetected && (
                            <span style={{ background: '#38bdf8', color: '#0f172a', fontSize: '0.65rem', fontWeight: 800, padding: '0.1rem 0.4rem', borderRadius: '4px', textTransform: 'uppercase' }}>
                              Recommended
                            </span>
                          )}
                        </div>
                  ...
                  <div style={{ padding: '2rem 1rem', textAlign: 'center', background: 'rgba(0,0,0,0.2)', borderRadius: '8px', border: '1px dashed var(--border-light)' }}>
                    <Bookmark size={24} style={{ color: '#fbbf24', opacity: 0.7, marginBottom: '0.5rem' }} />
                    <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>No custom templates saved yet.</p>
                    <p style={{ margin: '0.3rem 0 0', fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Click "Save as Template" on any configured firearm to create one.</p>
                  </div>
                ) : (
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: '8px',
                          cursor: 'pointer',
                          background: isSelected ? 'rgba(251, 191, 36, 0.15)' : 'rgba(255, 255, 255, 0.03)',
                      ...
                      >
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <strong style={{ fontSize: '0.95rem', color: isSelected ? '#fbbf24' : 'var(--text-primary)' }}>
                            {preset.name}
                          </strong>
                          <button
                            type="button"
              ...
              {(() => {
                ...
                if (presetTab === 'custom') {
                  const custom = customPresets.find(p => p.id === selectedPresetId) || customPresets[0];
                  if (custom) {
                    name = custom.name;
                  ...
                  const profile = MAINTENANCE_PROFILES[selectedPresetId] || MAINTENANCE_PROFILES.semi_pistol;
                  name = profile.name;
                ...
                if (!name && tasks.length === 0) {
                  return (
                    <div style={{ background: 'rgba(0,0,0,0.25)', padding: '2rem', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
                      <p style={{ color: 'var(--text-secondary)', margin: 0 }}>Select a profile or template to preview tasks.</p>
                    </div>
                  );
                }

                return (
                  <div style={{ background: 'rgba(0,0,0,0.25)', padding: '1.25rem', borderRadius: '10px', border: '1px solid var(--border-light)', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ marginBottom: '1rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.75rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                        <h4 style={{ margin: 0, fontSize: '1.1rem', color: 'var(--text-primary)' }}>{name}</h4>
                        {isDetected && (
                          <span style={{ background: 'rgba(56, 189, 248, 0.15)', color: '#38bdf8', border: '1px solid rgba(56, 189, 248, 0.3)', padding: '0.1rem 0.4rem', borderRadius: '4px', fontSize: '0.7rem', fontWeight: 600 }}>
                            Matches This Firearm
                          </span>
                        )}
                      </div>
                      <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{desc}</p>
                    </div>
                      ...
                      {tasks.map((task, idx) => (
                        <div key={idx} style={{ background: 'rgba(255,255,255,0.03)', padding: '0.65rem 0.85rem', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.05)' }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.2rem' }}>
                            <strong style={{ fontSize: '0.85rem', color: 'var(--text-primary)' }}>{task.task_name}</strong>
                            <div style={{ display: 'flex', gap: '0.3rem' }}>
                              <span style={{ background: 'rgba(56, 189, 248, 0.15)', color: '#38bdf8', padding: '0.1rem 0.4rem', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 700 }}>
                                Every {task.interval_rounds.toLocaleString()} rds
                              </span>
                              {task.interval_days && (
                                <span style={{ background: 'rgba(251, 191, 36, 0.15)', color: '#fbbf24', padding: '0.1rem 0.4rem', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 700 }}>
            ...
            <div className="modal-actions" style={{ marginTop: '1.25rem', paddingTop: '1rem', borderTop: '1px solid var(--border-light)', display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', flexWrap: 'wrap' }}>
              <button type="button" className="btn-secondary" onClick={() => setIsPresetModalOpen(false)}>
                Cancel
              </button>
              {firearm.maintenance_schedules && firearm.maintenance_schedules.length > 0 && (
                <button type="button" className="btn-secondary" onClick={() => handleLoadPreset(selectedPresetId, 'append', presetTab === 'custom')}>
                  Append to Existing Tasks
                </button>
              )}
              <button type="button" className="btn-primary" onClick={() => handleLoadPreset(selectedPresetId, 'replace', presetTab === 'custom')}>
        ...
        <div className="modal-overlay" style={{ zIndex: 1100 }}>
          <div className="modal" style={{ maxWidth: '480px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.75rem' }}>
              ...
              <button className="btn-icon" onClick={() => setIsSavePresetModalOpen(false)}>×</button>
            </div>

            <form onSubmit={handleSaveAsTemplate} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '0.4rem', fontWeight: 600, fontSize: '0.9rem' }}>
                  Template Name *
                </label>
              ...
              <div style={{ background: 'rgba(255,255,255,0.03)', padding: '0.75rem', borderRadius: '6px', fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                This template will capture all <strong>{firearm.maintenance_schedules?.length || 0} active schedule tasks</strong> from this firearm so you can apply them to any other firearm with 1-click.
              </div>

              <div className="modal-actions" style={{ marginTop: '0.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border-light)' }}>
                <button type="button" className="btn-secondary" onClick={() => setIsSavePresetModalOpen(false)}>Cancel</button>
      ...
      {isDossierOpen && firearm && (
        <div className="modal-overlay" style={{ zIndex: 1200 }}>
          <div className="modal" style={{ maxWidth: '980px', maxHeight: '92vh', display: 'flex', flexDirection: 'column', background: '#0f172a', color: '#f8fafc' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid var(--border-light)', paddingBottom: '0.75rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.5rem' }}>
                ...
                <button className="btn-icon" onClick={() => setIsDossierOpen(false)}>×</button>
              </div>
            </div>

            {/* Printable Document Body */}
            <div id="printable-firearm-dossier" style={{ overflowY: 'auto', flex: 1, padding: '1rem', background: '#ffffff', color: '#0f172a', borderRadius: '8px' }}>
              <style>{`
                @media print {
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #0f172a', paddingBottom: '1rem', marginBottom: '1.25rem' }}>
                <div>
                  <h2 style={{ margin: 0, fontSize: '1.6rem', color: '#0f172a' }}>{firearm.make} {firearm.model}</h2>
                  <div style={{ fontSize: '0.9rem', color: '#475569', marginTop: '0.2rem' }}>
                    Serial #: <strong>{firearm.serial_number || 'N/A'}</strong> &bull; Caliber: <strong>{firearm.caliber || 'N/A'}</strong> &bull; Action: <strong>{firearm.action_type || 'N/A'}</strong>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontWeight: 800, fontSize: '1.1rem', color: '#0284c7' }}>ARMORYVAULT DOSSIER</div>
                  <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Generated {new Date().toLocaleDateString()}</div>
                </div>
              </div>
              ...
              {(() => {
                ...
                const totalMalfunctions = (firearm.logs || []).reduce((sum, l) => sum + (Number(l.malfunctions) || 0), 0);
                const reliability = totalRounds > 0 ? (((totalRounds - totalMalfunctions) / totalRounds) * 100).toFixed(1) : '100.0';

                return (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.75rem', marginBottom: '1.5rem' }}>
                    <div style={{ background: '#f1f5f9', padding: '0.75rem', borderRadius: '6px', border: '1px solid #cbd5e1', textAlign: 'center' }}>
                      <div style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 600 }}>Lifetime Rounds</div>
                      <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#0f172a' }}>{totalRounds.toLocaleString()}</div>
                    </div>
                    <div style={{ background: '#f1f5f9', padding: '0.75rem', borderRadius: '6px', border: '1px solid #cbd5e1', textAlign: 'center' }}>
                      <div style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 600 }}>Service Spend</div>
                      <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#0f172a' }}>${totalMaintCost.toLocaleString(undefined, { minimumFractionDigits: 2 })}</div>
                    </div>
                    <div style={{ background: '#f1f5f9', padding: '0.75rem', borderRadius: '6px', border: '1px solid #cbd5e1', textAlign: 'center' }}>
                      <div style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 600 }}>Cleanings Logged</div>
                      <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#0f172a' }}>{totalCleanings}</div>
                    </div>
                    <div style={{ background: '#f1f5f9', padding: '0.75rem', borderRadius: '6px', border: '1px solid #cbd5e1', textAlign: 'center' }}>
                      <div style={{ fontSize: '0.75rem', color: '#64748b', textTransform: 'uppercase', fontWeight: 600 }}>Reliability Rating</div>
                      <div style={{ fontSize: '1.3rem', fontWeight: 800, color: '#059669' }}>{reliability}%</div>
                    </div>
                  </div>
                );
              })()}
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <h4 style={{ margin: '0 0 0.5rem', color: '#0f172a', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.3rem', fontSize: '1rem' }}>Firearm Specifications</h4>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem', fontSize: '0.85rem' }}>
                  <div><strong>Barrel Length:</strong> {firearm.barrel_length || 'N/A'}</div>
                  <div><strong>Finish:</strong> {firearm.finish || 'N/A'}</div>
                  <div><strong>Condition:</strong> {firearm.condition || 'N/A'}</div>
                  <div><strong>Acquired:</strong> {firearm.purchase_date || 'N/A'}</div>
                  <div><strong>Purchase Price:</strong> {firearm.purchase_price ? `$${firearm.purchase_price.toLocaleString()}` : 'N/A'}</div>
                  {firearm.is_nfa && <div><strong>NFA Type:</strong> {firearm.nfa_type || 'Yes'} (Stamp: {firearm.stamp_status || 'Approved'})</div>}
                </div>
              </div>
              ...
              {firearm.maintenance_schedules && firearm.maintenance_schedules.length > 0 && (
                <div style={{ marginBottom: '1.5rem' }}>
                  <h4 style={{ margin: '0 0 0.5rem', color: '#0f172a', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.3rem', fontSize: '1rem' }}>Active Service & Wear Schedules</h4>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                    ...
                    <tbody>
                      {firearm.maintenance_schedules.map((s, idx) => {
                        const totalRounds = firearm.logs?.filter(l => l.type === 'Range').reduce((sum, l) => sum + (Number(l.rounds_fired) || 0), 0) || 0;
                        const roundsSinceLast = Math.max(0, totalRounds - (s.last_performed_rounds || 0));
                        const isDue = roundsSinceLast >= s.interval_rounds;
                        return (
                          <tr key={idx} style={{ borderBottom: '1px solid #e2e8f0' }}>
                            <td style={{ padding: '0.4rem' }}><strong>{s.task_name}</strong></td>
                            <td style={{ padding: '0.4rem' }}>{s.interval_rounds.toLocaleString()} rds{s.interval_days ? ` / ${s.interval_days}d` : ''}</td>
                            <td style={{ padding: '0.4rem' }}>{s.last_performed_rounds.toLocaleString()} rds ({s.last_performed_date || 'N/A'})</td>
                            <td style={{ padding: '0.4rem', color: isDue ? '#dc2626' : '#059669', fontWeight: 600 }}>
              ...
              <div style={{ marginBottom: '1.5rem' }}>
                <h4 style={{ margin: '0 0 0.5rem', color: '#0f172a', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.3rem', fontSize: '1rem' }}>Complete Service & Range Ledger</h4>
                {(!firearm.logs || firearm.logs.length === 0) ? (
                  <p style={{ fontSize: '0.85rem', color: '#64748b', fontStyle: 'italic' }}>No service or range events recorded yet.</p>
                ) : (
                    ...
                    <tbody>
                      {firearm.logs.map((log, idx) => (
                        <tr key={idx} style={{ borderBottom: '1px solid #e2e8f0' }}>
                          ...
                          <td style={{ padding: '0.4rem' }}><strong>{log.type}</strong></td>
                          <td style={{ padding: '0.4rem' }}>{log.rounds_fired ? `${log.rounds_fired} rds` : '—'}</td>
                          <td style={{ padding: '0.4rem' }}>{log.installed_part_details || log.repaired_part || '—'}</td>
                          <td style={{ padding: '0.4rem' }}>{log.cost ? `$${Number(log.cost).toFixed(2)}` : '—'}</td>
              ...
              {attachedAccessories.length > 0 && (
                <div>
                  <h4 style={{ margin: '0 0 0.5rem', color: '#0f172a', borderBottom: '1px solid #e2e8f0', paddingBottom: '0.3rem', fontSize: '1rem' }}>Mounted Accessories, Optics & Suppressors</h4>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                    <thead>
                      <tr style={{ background: '#f1f5f9', borderBottom: '1px solid #cbd5e1', textAlign: 'left' }}>
                    ...
                    <tbody>
                      {attachedAccessories.map((acc, idx) => (
                        <tr key={idx} style={{ borderBottom: '1px solid #e2e8f0' }}>
                          ...
                          <td style={{ padding: '0.4rem' }}><strong>{acc.manufacturer} {acc.model}</strong></td>
                          <td style={{ padding: '0.4rem' }}>{acc.serialNumber || 'N/A'}</td>
                          <td style={{ padding: '0.4rem' }}>{acc.round_count ? `${acc.round_count.toLocaleString()} rds` : '0 rds'}</td>
                          <td style={{ padding: '0.4rem' }}>{acc.value ? `$${acc.value.toLocaleString()}` : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {firearm && (
        <AccessoryModal 
          ...
          onSave={() => loadFirearm()}
          editingId={null}
          initialData={{ mounts: [{ firearmId: Number(id), quantity: 1 }] }}
          firearms={[firearm]}
        />
      )}
