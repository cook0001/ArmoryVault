...
describe('RecoveryKeyModal Component', () => {
  beforeEach(() => {
    window.api = {
      ...
      getRecoveryCode: vi
        .fn()
        .mockResolvedValue('test-64-character-recovery-code-abcdef1234567890abcdef1234567890'),
  ...
  test('renders recovery key when open', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText('test-64-character-recovery-code-abcdef1234567890abcdef1234567890')
      ).toBeDefined();
    });
  ...
  test('opens regenerate confirmation and submits current password to re-key', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText('new-regenerated-64-character-recovery-code-9876543210fedcba9876543210')
      ).toBeDefined();
    });
  });
});
