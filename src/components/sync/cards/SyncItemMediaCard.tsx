...
export const SyncItemMediaCard: React.FC<SyncItemMediaCardProps> = ({
  ...
  if (item.type === 'universal_scan') {
    ...
    if (handloadPreview) {
      const cal =
        handloadPreview.cal ||
        handloadPreview.caliber ||
        handloadPreview.cartridge?.name ||
        'Handload';
      const bullet =
        handloadPreview.bullet ||
        handloadPreview.projectile?.name ||
        `${handloadPreview.grain || ''}gr`;
      const powder =
        handloadPreview.powder ||
        handloadPreview.powder_name ||
        handloadPreview.propellant?.name ||
        '';
