...
export const parseBarcodeData = (item: any, ammoList: Ammo[] = []): ParsedBarcodeResult => {
  ...
  if (typeof item === 'string') {
    ...
    if (
      trimmed.startsWith('{') &&
      (trimmed.includes('handload') ||
        trimmed.includes('LoadBench') ||
        trimmed.includes('ArmoryVault'))
    ) {
      try {
  ...
  if (
    ...
    const cal =
      rawJsonObj.cal ||
      rawJsonObj.caliber ||
      rawJsonObj.cartridge?.name ||
      rawJsonObj.cartridge ||
      '';
    const bullet = rawJsonObj.bullet || rawJsonObj.projectile?.name || rawJsonObj.projectile || '';
    const grainNum =
      rawJsonObj.grain ||
      rawJsonObj.projectile?.weight_grains ||
      (bullet ? parseInt(bullet) || undefined : undefined);
    ...
    const chargeNum =
      ...
      (rawJsonObj.powder
        ? parseFloat(rawJsonObj.powder.match(/([\d.]+)\s*gr/i)?.[1] || '') || undefined
        : undefined);
    ...
    const jump =
      rawJsonObj.projectile?.freebore_jump_in !== undefined
        ? rawJsonObj.projectile.freebore_jump_in
        : rawJsonObj.jump;
    const fps =
      rawJsonObj.simulated?.muzzle_velocity_fps ||
      rawJsonObj.simulated?.muzzleVelocityFps ||
      rawJsonObj.fps;
    const psi =
      rawJsonObj.simulated?.max_pressure_psi ||
      rawJsonObj.simulated?.maxPressurePsi ||
      rawJsonObj.psi;
    ...
    const targetRifle = rawJsonObj.metadata?.target_firearm
      ? `Rifle: ${rawJsonObj.metadata.target_firearm}`
      : '';
    ...
    const bestTitle =
      rawJsonObj.metadata?.name ||
      (titleParts.length > 0 ? titleParts.join(' ') : 'LoadBench Custom Handload');
    ...
    return {
      ...
      bestTitle,
      foundCost:
        rawJsonObj.economics?.cost_per_round_usd || rawJsonObj.costPerRound
