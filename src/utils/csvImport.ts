    ...
    try {
      ...
      if (items.length > 0 && typeof items[0] === 'object' && items[0] !== null) {
        ...
        const normalizedItems = items.map((item: any) => {
          ...
          const count =
            item.metadata?.batch_size || item.quantity || item.round_count || item.count || 50;
          const manufacturer =
            item.manufacturer || (item.format === 'loadbench_recipe' ? 'LoadBench' : 'Handload');
          const powder =
          ...
          const powderCharge =
            ...
            item.powderCharge ||
            '';
          const primer = (typeof item.primer === 'object' ? item.primer?.name : item.primer) || '';
          const primerType =
            item.primer_type ||
          ...
          const simInfo = simFps
            ? `LoadBench Sim (${simFps} fps${simPsi ? `, ${simPsi} psi` : ''}${obtNode ? ` • ${obtNode}` : ''})`
            : '';
          ...
          const targetRifle = item.metadata?.target_firearm
            ? `Rifle: ${item.metadata.target_firearm}`
            : '';
          const cbto = item.projectile?.cbto_in || item.cbto;
          const cbtoInfo = cbto ? `CBTO ${cbto}"` : '';
          const jump =
            item.projectile?.freebore_jump_in !== undefined
              ? item.projectile.freebore_jump_in
              : item.jump;
...
const AMMO_SYNONYMS: Record<keyof Ammo | string, string[]> = {
  caliber: ['caliber', 'cal', 'gauge', 'chambering', 'cartridge'],
  manufacturer: ['manufacturer', 'brand', 'make', 'mfg'],
  name: ['name', 'product name', 'line', 'ammo name', 'description'],
  bullet_type: ['bullet type', 'projectile', 'bullet', 'bullet_name'],
